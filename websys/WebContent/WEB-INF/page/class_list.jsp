...
<table class="layui-table" lay-data="{ url:'class/layuilist', toolbar:'#toptools',page:true, id:'form'}" lay-filter="form">
  <thead>
    <tr>
    <th lay-data="{type:'numbers'}">序号</th>
      <th lay-data="{field:'id' type:'hidden'}">ID</th>
      <th lay-data="{field:'cclass' }">菜系名</th>
  ...
  <div class="layui-input-inline">
    <input type="text" name="name"  placeholder="请输入菜系" autocomplete="off" class="layui-input">
  </div>
...
<!-- start 新增信息窗口 -->
<div id='addClassBox' style="display: none">
<form class='layui-form' lay-filter="addClassForm">
<div class="layui-form-item">
    <label class="layui-form-label">菜系名</label>
    <div class="layui-input-inline">
      <input type="text" name="cclass"  class="layui-input">
    </div>
  </div>
<div class="layui-form-item">
    <label class="layui-form-label">菜系编号</label>
    <div class="layui-input-inline">
...
<!-- end 新增信息窗口 -->
<!-- start 编辑信息窗口 -->
<div id='editClassBox' style="display: none">
<form class='layui-form' lay-filter="editClassForm">
	<input type='hidden' name='id'><!-- 数据设置为用户不可见 -->
<div class="layui-form-item">
    <label class="layui-form-label">菜系名</label>
    <div class="layui-input-inline">
      <input type="text" name="cclass"  class="layui-input">
...
table.on("toolbar(form)",function(obj){
	 ...
	 if(event=="add"){
		...
		layer.open({
			...
			btn1:function(){
				...
				var formdata=form.val("addClassForm");
				$.ajax({
					url:"class/add",
					data:formdata,
		...
		var text_id=$("#topbox [name='id']").val();
		...
		var text_id=$("#topbox [name='id']").val(text_id);
	}
});
  ...
  table.on('tool(form)', function(obj){
    ...
    var data = obj.data;
	  ...
	  if(obj.event ==='edit'){
		  //将当前行数据设置到表单中
		  form.val("editClassForm",data);
		  //显示弹出层  大括号设置参数
		  layer.open({
			  title:'编辑',
			  type:1,
			  area:['400','260'],
			  content:$("#editClassBox"),
			  btn:["保存","取消"],
			  btn1:function(){
				  var data=form.val("editClassForm");
				  $.ajax({
						url:"class/update",
						data:data,
						success:function(result){
    	...
    	//业务逻辑
      layer.confirm('真的删除行么,菜系包含的所有菜品都会删除!',{icon:6,title:"提示"}, function(index){//提示用户确认删除
    	  //将该行数据发送给servlet,servlet调用其他工具删除该数据
    	  $.ajax({
    		 
    		  url:"class/deletelist",
    		  data:{
    			  id:data.id,
    		  },
